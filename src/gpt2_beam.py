...
parser.add_argument('--model_card', default='gpt2.sm', choices=['gpt2.sm', 'gpt2.md', 'gpt2.lg', 'gpt2.xl'],
                    help='model names')
...
parser.add_argument('--hyperparam', action='store_true', help='workdir based on parameters in hyperparam')
...
if __name__ == '__main__':
    ...
    if args.init_checkpoint is not None:
        ...
        if args.hyperparam:
            folder = args.init_checkpoint
            latest_file = max([os.path.join(folder, f) for f in os.listdir(folder)], key=os.path.getctime)
            print(f'using model in {latest_file}')

            args.init_checkpoint = latest_file
